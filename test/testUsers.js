...
describe('Testing User collection:', function(){
  ...
  before(function(done){
    mongoose.connect('mongodb://localhost/lectureviewer_db', function (err, res) {
      if(err) {
    ...
    db.user.dropUserDatabase(function() {
      db.user.createUser('test@test.com', 'password', 'username', 'role', function (err, doc) {
        testUser = doc;
   ...
   */
  after(function(done) {
    db.user.deleteUserById(testUser._id, function(err, user) {
      assert.equal(err, null);
  ...
  it('retrieves user Role by ObjectID', function(done) {
    db.user.getUserRoleById(testUser._id, function(err, role) {
      assert.equal(err, null);
  ...
  it('set username by ObjectId', function(done) {
    db.user.setUsernameById(testUser._id,'newUsername', function(err, doc) {
      assert.equal(err, null);
  ...
  it('set name by ObjectId: firstname, lastname', function(done) {
    db.user.setNameById(testUser._id, 'firstname','lastname', function(err, doc) {
      assert.equal(err, null);
      assert.notEqual(doc, null);
  ...
  it('Add notifications by ObjectId: id, type, title, url, date', function(done) {
    db.user.notification.addNotificationById(
        testUser._id,
  ...
  it('retrieves notifications by ObjectId', function(done) {
    db.user.notification.getAllNotificationsById(testUser._id, function(err, notifications) {
      assert.equal(err, null);
  ...
  it('Add bookmark by ObjectId: id, title, url', function(done) {
    db.user.bookmark.addBookmarkById(testUser._id, {title: "title",url:"url"}, function(err, user) {
      assert.equal(err, null);
      assert.notEqual(user, null);
  ...
  it('retrieves bookmark by ObjectId: ObjectId', function(done) {
    db.user.bookmark.getBookmarksById(testUser._id, function(err, bookmarks) {
      assert.equal(err, null);
  ...
  it('retrieves user by ID: ID', function(done) {
    db.user.getUserById(testUser._id, function(err, user) {
      assert.equal(err, null);
